...
For my next post on Self Organizing Maps I started researching different ways on how to determine the quality of a map. Firstly, I found [this article](https://csce.ucmss.com/cr/books/2018/LFS/CSREA2018/ICD8051.pdf) entilted "Evaluating Self-Organizing Map Quality Measures as Convergence Criteria", which contains a couple of different measures on approaching the problem. Alongside it, I found R code in this [thesis](https://digitalcommons.uri.edu/cgi/viewcontent.cgi?article=2013&context=theses), which was used in the aforementioned article. The problem was that this code wasn't available on GitHub or anywhere on the Internet, which could be used easily. 
Therefore, I decide to create a R package consisting of the funtions that are created for the "Evaluating Self-Organizing Map Quality Measures as Convergence Criteria" article purposes.
...
The most time consuming part, which took 10-15 minutes was to make sure that all whitespaces are correctly copied. Here, I mean cases when a dot `.`{.r} is copied with a whitespace at the beginning ` .`{.r}, at the end ` .`{.r} or both whitespaces are added ` . `{.r}. This was the case for many characters as `",", "<", ">", ")", "("`, etc.
Additionally, I used the `library(styler)` to style the R script properly. I highly recommend this package, which on RStudio works as an Addin and is extermely efficient and easy to use. Only selecting the script and pressing the `style_selection()`{.r} option from the Addin is needed to neatly style the code.

## Creating the R Package structure

I decided to use the following way of creating a R package, which is to use the 'library(usethis)'{.r} functionality. [Here](https://www.rdocumentation.org/packages/usethis/versions/1.5.1) all of the set up is well-written. The main idea of this approach is create a skelton of a package and then add to it whatever is needed.

>In this article I won't desribe the structure of a R package. An interested reader is adviced to take a look at the [R packages](http://r-pkgs.had.co.nz/) book created by Hadley Wickham. In this book one can find the full and complete description of R pakcage, its contents and many useful tips.

Therefore, I exectued the following code:
...
After a copule of seconds (maybe one minute in the worst case) - viola, the R package is there!
...
Setting up following files is also not necessary. However, as I would like to maintain this packageand keep track of any changes in the package I decided to add following lines:
...
To finish up setting the package - once the pakcage is craeted, I have to describe its contents briefly in the `DESCRIPTION.txt` file. 
...
I am copying each R function that I previously saved in a `som_quality_functions.r`{.r} to a new file, which has the name of the function. I am saving each file in the `/R` folder in the `SomQualityCheck` project directory. As previously noted, for each new R script at the beginning I have to create the `Roxygen2` notes. Once the package is created, it will serve as a documentation. Such documentation will greatly help to maintain the package and save time when debugging or trying to redisover after some time what has been written in the script.
Note that for some of the functions I set the description to `TODO`, as the main purpose currently is to add to the package all functions. One has to keep in mind that all paramteres have to be described in the documentation. Otherwise during the package build an error will occur.

## Adding C++ functions

There are a couple of cpp functions that need a different treatmentto standard R functions like:
...
I recommend to check the result of this blog article at my [GitHub](https://github.com/pawelsobala/SomQualityCheck). The next steps are defenitely to:
